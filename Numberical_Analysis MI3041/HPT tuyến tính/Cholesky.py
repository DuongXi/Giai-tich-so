...
def choleski(matrix):
    ...
    a = (matrix[0:size,0:size]).copy()
    a = np.transpose(a)*a
    print(a)
    
    for i in range (size):
        if i == 0:
            q[i,i] = math.sqrt(a[i,i])
            if q[i,i] == 0:
                print("Ma trận A suy biến, không thoả mãn điều kiện")
                return
            q[i,1:] = a[i,1:]/q[i,i]
        else:
            q[i,i] = math.sqrt(a[i,i] - sum(q[0:i,i]**2))
            if q[i,i] == 0:
                print("Ma trận A suy biến, không thoả mãn điều kiện")
                return
            for j in range(i+1,size):
                q[i,j] = (a[i,j]-(q[0:i,i] @ np.transpose(q[0:i,j])))/q[i,i]
                
    matrix[0:size,0:size] = np.transpose(q)
